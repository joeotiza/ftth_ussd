...
Class Action {
	...
	function delete_case(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM `cases_reported` where id= ".$id);
		if($delete)
			return 1;
	}
	function delete_change(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM `plan_change` where id= ".$id);
		if($delete)
			return 1;
	}
	function delete_chat(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM `chat` where id= ".$id);
		if($delete)
			return 1;
	}
